...
v21では、運用監視で画面を見続ける人の使いやすさを優先して、以下を追加しました。

- **更新差分ハイライト**: Auto refresh / 手動Refresh後に、Status、Current Run、History、Pending、CloseTime、Run countなどが変わったWorkflow/Runを一時的に強調表示します。
- **スクロール位置の維持**: 自動更新後も、ページのスクロール位置とテーブルの横スクロール位置を可能な限り維持します。
- **Pin / Watch list**: 気になるWorkflowをWatch listに固定できます。Pin済みWorkflowは上位に表示され、`Watch list only` で絞り込みできます。Pin状態はブラウザのlocalStorageに保存されます。
- **Problem first summary**: Run DetailsのExecution Motion上部に、失敗、待機中、Child Workflow問題、最長Activityなど、先に見るべき項目をまとめて表示します。

Workflows一覧の基本思想は変えていません。初期状態ではExecution Treeは閉じたままで、ユーザーが開いたツリーだけが更新後も開いたまま残ります。

## v22: Run Detail の Activity 全件表示

Run Detail の `Execution Motion` で Activity が途中で途切れて見える問題を修正しました。
選択された Run の History は省略せず取得し、Activity rows、Child workflow rows、Operator markers を全件表示します。
