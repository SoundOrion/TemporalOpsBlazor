...
Execution Motion の Activities は、監視・障害一次切り分けで視認性が重要なため、密集した1行表示から個別行表示に変更しました。各Activityは Status、Start、End、Duration、Event ID範囲を持ち、クリックするとInspectorに詳細が表示されます。Full Historyを読む前に、どのActivityが遅延・失敗・待機しているかを判断するための中間ビューとして使います。
...
## v22 Run Detail Activity full rendering

Run Detail の `Execution Motion` では、選択された Run の History を省略せず取得し、Activity rows / Child workflow rows / Operator markers を全件表示します。
`HistoryEventLimit` は Grouped Detail などの軽量表示向けの上限として扱い、Run Detail の Activity 視認性を損なわないようにしています。
